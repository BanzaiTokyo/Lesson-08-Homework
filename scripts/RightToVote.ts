...
async function main() {
  ...
  const chairVoter = (await publicClient.readContract({
    address: contractAddress,
    abi,
    functionName: "chairperson",
    args: [],
  })) as any[];
  const newVoter = voterAddress;
  console.log("Do you want to give the right to vote to", newVoter, "?");
  ...
  stdin.addListener("data", async function (d) {
    if (d.toString().trim().toLowerCase() != "n") {
      ...
      console.log(newVoter, "is eligible to vote");
